...
public class CoverShould
{
    ...
    public void BeAbleToCreateCover()
    {
        var actual = Cover.Create(
            new CreateCover(DateOnly.FromDateTime(DateTime.Now).AddDays(1),
                DateOnly.FromDateTime(DateTime.Now).AddYears(1), CoverType.Tanker),
    ...
    public void NotCreateCoverWithInsurancePeriodExceedOneYear()
    {
        var actual = Cover.Create(
            new CreateCover(DateOnly.FromDateTime(DateTime.Now).AddDays(1),
                DateOnly.FromDateTime(DateTime.Now).AddYears(2), CoverType.Tanker),
            new DefaultPremiumStrategy());
        new DefaultPremiumStrategy();
